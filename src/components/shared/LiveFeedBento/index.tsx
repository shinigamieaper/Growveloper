...
/* ─── Theme-aware accent variants (same system as CaseStudyCard) ─── */
...
/* ─── Thumbnail header (fills top of bento card) ─── */
function ThumbnailHeader({
  ...
  src: string;
  alt: string;
  type: "blog" | "youtube" | "tiktok";
  featured: boolean;
  variant: (typeof accentVariants)[number];
    ...
    >
      {src ? (
        ...
        <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-bg-tertiary to-bg-secondary">
          <ContentIcon type={type} className={cn("h-10 w-10 opacity-30", variant.text)} />
        </div>
      )}
      ...
      <span
        className={cn(
          "absolute left-3 top-3 z-10 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wide backdrop-blur-sm",
          variant.pill,
        )}
      >
        {labels[type]}
      ...
      {(type === "youtube" || type === "tiktok") && (
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (onVideoClick && videoUrl && platform) onVideoClick(videoUrl, platform);
          }}
...
export function LiveFeedBento({
  ...
  const allCards = items.slice(0, 3);

  return (
    <div className="w-full">
      {/* Section header */}
      {(sectionTitle || seeAllUrl) && (
        <div className="mb-8 flex items-end justify-between">
          ...
          {seeAllUrl && (
            <LinkPreview url={seeAllUrl} className="text-sm font-medium text-brand-mid hover:text-brand-light">
              {seeAllLabel}
            </LinkPreview>
          )}
        </div>
      ...
      <BentoGrid className="md:auto-rows-[22rem]">
        {allCards.map((data, i) => {
          ...
          const cardInner = (
            ...
            >
              ...
              >
                ...
                {/* Bottom slot — icon + title + description (slides on hover) */}
          ...
          const colClass = isFeatured ? "md:col-span-2" : "md:col-span-1";
